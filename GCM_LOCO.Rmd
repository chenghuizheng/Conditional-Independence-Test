...
run_simulation_a <- function(num_simulations) {
  ...
  for (i in 1:num_simulations) {
   ...
   #y1 <- 1.5*data_indep$X1 + 1.5*data_indep$X2 + 2*data_indep$X3 + 2*data_indep$X4+
    ...
    # Run LOCO
    LOCO_results <- LOCO_all(data_indep, "regr.randomForest", "y")
...
start <- proc.time()
results_all11 <- run_simulation_a(10)
print( proc.time() - start )



write.csv(results_all11, "simulation_locoall1.csv", row.names = FALSE)
...
```{r}
results_all1$Features <- factor(results_all1$Features, levels = paste0("X", 1:20))
 pl1 <- ggplot(results_all1, aes(x = Features, y = P.Value)) +
...
print( proc.time() - start )
write.csv(results3, "simulation_locoall3.csv", row.names = FALSE)
#       user  system elapsed 
...
write.csv(results_d, "simulation_locoall4.csv", row.names = FALSE)
#   user  system elapsed 
